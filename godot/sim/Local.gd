...
const QUAKE_RATE := 0.02     # 构造应力累积率(逐日)
const QUAKE_THR := 1.0       # 地震阈值(张弛)
const LITH_THR := 15.0       # 沉积固结成岩阈值
...
# 局部地质(逐日):地震(构造应力张弛)+ 沉积固结成沉积岩(按成分分类,守恒:dep→岩)
func _geo_local_step(L: Dictionary) -> void:
	if L["kind"] == "cave": return
	L["tecStress"] = float(L["tecStress"]) + QUAKE_RATE
	if float(L["tecStress"]) > QUAKE_THR:
		L["tecStress"] = 0.0
		L["quakes"] = int(L["quakes"]) + 1
		L["elev"] = float(L["elev"]) + 0.5            # 地震微抬升/错断
	var dep: Array = L["dep"]
	var tot := 0.0
	for e in Sim.NE: tot += dep[e]
	if tot > LITH_THR:                                # 沉积超阈→固结成沉积岩(岩性按成分)
		L["lith"] = _classify_sed(dep)
		L["lithified"] = true
		for e in Sim.NE:
			var lk: float = dep[e] * 0.5              # 一半锁进岩石(守恒:dep→rockE3)
			dep[e] -= lk; rockE3[e] += lk

func _classify_sed(dep: Array) -> String:            # 移植 world.html classifySed:按沉积成分定岩性
	var salt: float = float(dep[0]) + float(dep[5]) + float(dep[6])   # Na+Cl+SO4
	var carb: float = float(dep[1]) + float(dep[7])                   # Ca+碳酸盐
	var sil: float = float(dep[8])                                    # SiO2
	if salt > carb and salt > sil: return "蒸发岩"
	if carb > sil: return "石灰岩"
	return "砂岩"
