...
var latGene := PackedFloat64Array() # 每格 N_LAT 潜在基因(索引 k*N_LAT+l;起点0→表达0.5中性)
	...
	for j in NLat:
		...
		for i in NLon:
			...
			if N[k] > 1e-3:
				...
				for _l in N_LAT:   # B2 潜在基因梯度演化(向匹配环境的表达爬)
					var _lp := 1.0 / (1.0 + exp(-latGene[k * N_LAT + _l]))
					latGene[k * N_LAT + _l] += LAT_ETA * dl * LAT_EFF * _latsig(_l, j) * _lp * (1.0 - _lp)
...
func geneMutate() -> void:   # B层:逐地质年给有生命格的基因加种子化随机扰动(探索,逃局部最优;不碰物质守恒)
	...
	for k in SZ:
		...
		for l in N_LAT: latGene[k * N_LAT + l] += MUT_K * _gnoise(k, 100 + l, geoT)   # B2 潜在基因突变
...
func _latGrow(k: int, j: int) -> float:   # 潜在性状弱耦合生长:表达匹配环境→增益、否则代价(功能涌现的选择压)
	...
	for l in N_LAT:
		...
		f += LAT_EFF * (2.0 * lp - 1.0) * _latsig(l, j)
	return clampf(f, 0.6, 1.4)
...
func spinUp() -> void:
	...
	latGene = PackedFloat64Array(); latGene.resize(SZ * N_LAT)   # B2 潜在基因(中性起点)
	_devW = []
	for _m in GENE_K:
		var _row := []
