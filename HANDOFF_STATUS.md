...
**生物进化(用户最新方向,理想=长期演化出像地球各地质纪的复杂性状+行为生物):**
- **E3 行为逻辑**(未做):现在生物只有形态/生理性状,**没有行为**(觅食/迁徙/避敌/社群策略)。这是用户明确要的"行为逻辑"。
- **E4 地质年代可辨生物**(未做):把复杂度性状接进体制判定 `World.bodyPlan()`(现只用 Sym/Seg/Limb/Axis→刺胞/环节/节肢/脊索/蠕虫),让长期演化涌现出像各纪的生物 + 给生物起"年代感"的表型/命名。复杂性状地基 E1 已通,这步可接上去。
- **食物网稳定性**:食肉者是 episodic(全局同步 boom-bust + 大灭绝是设计)。若要更稳的三级共存,是非线性动力学难题(空间异步/避难所),非简单调参。别钻"静态稳定"牛角尖。
